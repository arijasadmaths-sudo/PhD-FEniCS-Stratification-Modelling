#!/usr/bin/env python3
"""Axisymmetric 70 cc/min: the finer-mesh 2-second scalar-flux check.

Run: python3 axisymmetric_70_fine_flux_2s.py --output NEW_EMPTY_DIRECTORY
Tests without FEniCS: python3 axisymmetric_70_fine_flux_2s.py --self-test

Legacy FEniCS 2019.1, NumPy and SciPy; exactly one MPI rank.
Each original physical radial and vertical interval is halved, giving
50,184 triangles. Physical parameters, the 2 s inlet ramp, dt=0.001 s and
semi-implicit Oseen IPCS equations match the completed coarse-mesh check. No molecular scalar
...
def _verification_refine_nodes(nodes, factor):
    ...
    if factor == 1:
        return nodes.copy()
    if factor != 2:
        raise ValueError("This matched check supports refinement factors 1 and 2")
...
ARGS = parse_args()
OUTPUT = ARGS.output
state = {"status": "starting", "target_time_s": 2.0, "case": "balanced_flux_fine_2s"}
...
try:
    ...
    if ncells != 50184:
        raise RuntimeError("Unexpected mesh: this finer-mesh diagnostic must use 50184 cells")
    ...
    config = {
        "case": "balanced_flux_fine_2s", "target_time_s": T_final, "dt_s": dt_value,
        ...
        "mesh_refinement": 2, "fresh_start": True, "auto_restart": False,
        ...
        "claim_scope": "Two-second comparison with the completed coarse flux check; not validation of 50/1000 s results",
    }
    ...
    def save_snapshot(name):
        ...
        with temp.open("wb") as handle:
            np.savez_compressed(handle,
                case=np.asarray("balanced_flux_fine_2s"), time_s=np.asarray(accepted_time), step=np.asarray(accepted_step),
    ...
    write_json(OUTPUT, "verification_status.json", state)
    print("Completed the finer-mesh 2-second diagnostic. The supplied launcher now assembles the RETURN folder and ZIP for upload.", flush=True)
