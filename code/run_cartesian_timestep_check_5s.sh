#!/bin/bash
#SBATCH --job-name=cart_timestep_5s
...
#SBATCH --output=cartesian_timestep_check_5s_%j.out
#SBATCH --error=cartesian_timestep_check_5s_%j.err

# Put this file and cartesian_ipcs_verification_5s.py in the same directory.
# Submit from that directory: sbatch run_cartesian_timestep_check_5s.sh
# 120 x 60 cells; dt=0.0005 s; 10000 steps. Five physical seconds from rest.
...
export CARTESIAN_FV_OUTPUT="/user/work/kn22417/cartesian_timestep_check_5s_${SLURM_JOB_ID:?Missing Slurm job ID}"

echo "Cartesian timestep verification: 120 x 60 cells; dt=0.0005 s; 10000 steps; NO CLIPPING"
...
srun --mpi=pmi2 python3 -u "$fv_script" --case timestep --output "$CARTESIAN_FV_OUTPUT"
