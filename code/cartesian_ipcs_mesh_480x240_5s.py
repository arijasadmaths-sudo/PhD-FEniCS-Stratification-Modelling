#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""480 x 240 Cartesian mesh verification: five physical seconds from rest.

Run with legacy FEniCS/DOLFIN 2019.1, NumPy, SciPy and mpi4py, ONE MPI rank:
    python3 cartesian_ipcs_mesh_480x240_5s.py --output NEW_EMPTY_DIRECTORY
Self-tests (NumPy/SciPy only):
    python3 cartesian_ipcs_mesh_480x240_5s.py --self-test

Cases (each starts from rest and ends at exactly t=5 seconds):
    mesh480:  480 x 240 cells, dt=0.001 s, 5000 steps (DEFAULT).
...
The new default mesh has 1.25 mm cells and eight faces across the 10 mm slot.
Compare its fv_final.npz with the previous 240 x 120 result at t=5 seconds.
The earlier cases remain available through --case for exact reproduction.
...
PREVIOUS_VERIFICATION_SHA256 = "494745c08645491e1d69e5ddb99e1eb66a2350c51486e094840e136b1164ef29"
VERIFICATION_CASES = {
    "baseline": {"nx": 120, "ny": 60, "dt_s": 0.001},
    "mesh": {"nx": 240, "ny": 120, "dt_s": 0.001},
    "mesh480": {"nx": 480, "ny": 240, "dt_s": 0.001},
...
class KernelTests(unittest.TestCase):
    def test_verification_case_time_and_nozzle_alignment(self):
        expected = {"baseline": (120,60,.001,5000,250,100,2),
                    ...
                    "mesh480": (480,240,.001,5000,250,100,8),
    ...
    def test_refined_grid_projection_uniform_and_source_budget(self):
        g = BalancedFluxGrid(480, 240, .6, .3)
        fx, fy, mask = production_boundary_fluxes(g)
        np.testing.assert_array_equal(mask, aligned_nozzle_mask(480,.6,.01))
        self.assertEqual(int(mask.sum()), 8)
        self.assertAlmostEqual(float(fy[0,mask].sum()), (2./3.)*.001*.01, places=16)
        rng = np.random.RandomState(170)
        fx[:,1:-1] = rng.normal(scale=1.e-5, size=(240,479))
        fy[1:-1,:] = rng.normal(scale=1.e-5, size=(239,480))
        fx, fy, diagnostics = g.balance(fx,fy)
        uniform, d = g.step(np.full((240,480),.37),fx,fy,.001,1.e-7,mask,boundary_c=.37)
        np.testing.assert_allclose(uniform,.37,atol=1.e-13,rtol=0.)
        c, d = g.step(np.ones((240,480)),fx,fy,.001,1.e-7,mask,
...
def run_pilot(args):
    ...
    try:
        ...
        config = {"method":"Oseen IPCS + conservative rectangular FV scalar",
                  ...
                  "previous_verification_sha256":PREVIOUS_VERIFICATION_SHA256,
        ...
        print("Pilot completed. Send fv_final.npz, scalar_budget.csv, budget_configuration.json and run_status.json for review.",flush=True)
...
def main():
    ...
    parser.add_argument("--case",choices=tuple(VERIFICATION_CASES),default="mesh480",
                        help="Matched five-second verification case (default: mesh480, 480 x 240 cells)")
