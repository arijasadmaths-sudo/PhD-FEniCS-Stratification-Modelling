#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Short Cartesian compact-core control and finer mesh checks.

compact312p5: h=.0003125 m, 268 x 194 FV cells (control).
compact156p25: h=.00015625 m, 406 x 327 FV cells (finer).
Both start from rest and reach exactly t=5 s at dt=.001 s.

Uniform core x=[.28,.32] m, z=[0,.04] m; full domain [0,.6] x [0,.3] m.
Outer spacing grows smoothly to <=5 mm; the 10 mm inlet has 32/64 faces.
First compare compact312p5 with the completed larger-core focus312p5 run.
Only after this control is satisfactory interpret compact156p25 versus
compact312p5 as a further short spatial-sensitivity check. The velocity mesh
changes beyond the compact core, even where the scalar deficit is negligible.

Physical parameters, Oseen-IPCS equations, flux residual correction and scalar
transport are unchanged from the completed focus312p5 source. Scalar transport
remains implicit conservative first-order upwind, D=1e-7 m^2/s, with physical
cell areas/transmissibilities and interior-only balancing of face fluxes.
No clipping or mass rescaling. No late-time or mesh-independence claim.

Legacy DOLFIN 2019.1, one MPI rank:
  python3 cartesian_compact_5s.py --case compact312p5 --output NEW_DIRECTORY
  python3 cartesian_compact_5s.py --self-test
  python3 cartesian_compact_5s.py --describe

An interrupted run can resume using --resume /absolute/path/checkpoint_latest.npz
with exactly this script, the same case and compatible FE DOF layout.
Snapshots and checkpoints are written every .25 simulated seconds; checkpoints
contain the full FE velocity/pressure, mesh, scalar and cumulative budgets.
SIGTERM/SIGUSR1 requests an accepted checkpoint after the current completed step.
A new job writes a separate output segment and never overwrites the old run.
Checkpoints from any focus case or the other compact case are incompatible.
"""
import argparse
import csv
import datetime
...
def run_case(args):
    ...
    try:
        ...
        config={'method':'Oseen IPCS + conservative FV on graded rectangular cells',
            ...
            'interpretation':'First compare compact312p5 with completed larger-core focus312p5 at 5 s to control for the changed outer velocity mesh; only then interpret compact156p25 versus compact312p5 as short spatial sensitivity. No late-time or global mesh-independence claim'}
...
class KernelTests(unittest.TestCase):
    ...
    def test_atomic_checkpoint_restart_and_wrong_case_rejection(self):
        ...
        data=dict(metadata_json=json.dumps({'format_version':1,'case':'compact312p5','fingerprint':'test'}),
