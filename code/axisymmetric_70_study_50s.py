#!/usr/bin/env python3
"""Axisymmetric matched mesh/timestep study at Q=70 cc/min, through 50 s.

Cases: coarse (12546 cells, dt=.001), fine (50184 cells, dt=.001),
fine_half (50184 cells, dt=.0005), extra_fine (200736 cells, dt=.001),
fine_quarter (50184 cells, dt=.00025),
extra_fine_half (200736 cells, dt=.0005),
extra_fine_quarter (200736 cells, dt=.00025; new launcher pauses at 5 s).
Every case starts from salt water/rest unless explicitly resumed.
The completed corrected 2-second solver's momentum and scalar schemes are
retained, including its 2-second inlet ramp and one-step raw-flux shadow.
No molecular scalar diffusion, clipping or redistribution is introduced.

Requires legacy FEniCS 2019.1, NumPy and SciPy; exactly one MPI rank.
See README.txt for submission, matched outputs, restart and return ZIPs.
"""
...
import signal
import shutil
from study_checkpoint import atomic_npz, fingerprint, write_checkpoint, load_checkpoint, carry_budget
...
REPAIR_PARENT_SOURCE_SHA256 = "03cf644a84108531642084946e5a93e79cfdc2f72211e631e6e5af5f7feb5b95"


def compatible_checkpoint_identity(path, current_identity):
    """Allow this one audited scalar-check repair without rewriting old data.

    The original checkpoint loader still checks every identity item, array,
    accepted step, time and budget. Only the exact reviewed parent source is
    compatible, and only for extra_fine_half with every other identity equal.
    """
    with np.load(str(path), allow_pickle=False) as archive:
        saved = json.loads(str(archive["metadata_json"].item())).get("identity")
    if saved == current_identity:
        return current_identity
    parent_identity = dict(current_identity)
    parent_identity["source_sha256"] = REPAIR_PARENT_SOURCE_SHA256
    if current_identity.get("case") == "extra_fine_half" and saved == parent_identity:
        return parent_identity
    raise ValueError("Checkpoint is neither this source nor its exact compatible repair parent")


def compatible_history_sources(identity):
    sources = {identity["source_sha256"]}
    if identity.get("case") == "extra_fine_half":
        sources.add(REPAIR_PARENT_SOURCE_SHA256)
    return sources
...
def _verification_refine_nodes(nodes, factor):
    """Bisect physical r and z intervals, retaining every original node."""
    nodes = np.asarray(nodes, dtype=float)
    if nodes.ndim != 1 or len(nodes) < 2 or not np.all(np.isfinite(nodes)) or np.any(np.diff(nodes) <= 0):
        raise ValueError("Mesh nodes must be a finite, strictly increasing vector")
    if factor == 1:
        return nodes.copy()
    if factor == 4:
        return _verification_refine_nodes(_verification_refine_nodes(nodes, 2), 2)
    if factor != 2:
        raise ValueError("This matched check supports refinement factors 1, 2 and 4")
    refined = np.empty(2*len(nodes)-1)
    refined[::2] = nodes
    refined[1::2] = (nodes[:-1]+nodes[1:])/2.0
    return refined


def fine_mesh_self_test():
    radial = np.r_[np.linspace(0, 0.001, 3), np.linspace(0.001, 0.01, 10)[1:], np.linspace(0.01, 0.30, 31)[1:]]
    vertical = np.r_[np.linspace(0, 0.03, 16), np.linspace(0.03, 0.30, 37)[1:]]
    rr = _verification_refine_nodes(radial, 2)
    zz = _verification_refine_nodes(vertical, 2)
    np.testing.assert_array_equal(rr[::2], radial)
    np.testing.assert_array_equal(zz[::2], vertical)
    np.testing.assert_allclose(np.diff(rr)[::2], np.diff(radial)/2, rtol=5e-14)
    np.testing.assert_allclose(np.diff(zz)[::2], np.diff(vertical)/2, rtol=5e-14)
    assert len(rr) == 83 and len(zz) == 103
    assert 6*(len(rr)-1)*(len(zz)-1) == 50184
    assert np.count_nonzero(rr == 0.001) == 1
    assert np.count_nonzero(zz == 0.03) == 1
    assert rr[-1] == 0.30 and zz[-1] == 0.30
    print("PASS: physical r/z intervals halved, nozzle edge retained, 50184 triangles")
    rrr = _verification_refine_nodes(radial, 4)
    zzz = _verification_refine_nodes(vertical, 4)
    np.testing.assert_array_equal(rrr[::2], rr)
    np.testing.assert_array_equal(zzz[::2], zz)
    np.testing.assert_array_equal(rrr[::4], radial)
    np.testing.assert_array_equal(zzz[::4], vertical)
    np.testing.assert_allclose(np.diff(rrr).reshape(-1, 4),
                               np.repeat((np.diff(radial)/4)[:, None], 4, axis=1), rtol=1e-13)
    np.testing.assert_allclose(np.diff(zzz).reshape(-1, 4),
                               np.repeat((np.diff(vertical)/4)[:, None], 4, axis=1), rtol=1e-13)
    assert len(rrr) == 165 and len(zzz) == 205
    assert 6*(len(rrr)-1)*(len(zzz)-1) == 200736
    assert len(rrr)*len(zzz)+2*(len(rrr)-1)*(len(zzz)-1) == 100737
    assert np.count_nonzero(rrr == 0.001) == 1
    assert np.count_nonzero(zzz == 0.03) == 1
    assert rrr[-1] == 0.30 and zzz[-1] == 0.30
    print("PASS: fine physical r/z intervals halved again, nozzle edge retained, 200736 triangles")
...
def parse_args():
    ...
    p.add_argument("--case", choices=("coarse", "fine", "fine_half", "extra_fine", "fine_quarter", "extra_fine_half", "extra_fine_quarter"))
    p.add_argument("--resume", help="Accepted checkpoint_latest.npz; keep its sibling CSV and fields")
    p.add_argument("--stop-after-steps", type=int, default=0,
                   help="Gracefully pause after this many steps in this allocation; zero disables")
    p.add_argument("--end-time", type=float, default=50.0,
                   help="Physical target; launchers set 50 s or 5 s; shorter targets permit smoke checks")
    ...
    if args.self_test:
        ...
        fine_mesh_self_test()
        sys.exit(0)
    if not args.output or not args.case:
        p.error("--case and --output are required for a run")
    if args.stop_after_steps < 0:
        p.error("--stop-after-steps must be nonnegative")
    dt_value = {"fine_half": 0.0005, "fine_quarter": 0.00025, "extra_fine_half": 0.0005, "extra_fine_quarter": 0.00025}.get(args.case, 0.001)
    if (not np.isfinite(args.end_time) or args.end_time <= 0 or args.end_time > 50
            or round(args.end_time/dt_value) < 1
            or abs(args.end_time/dt_value-round(args.end_time/dt_value)) > 1e-8):
        p.error("--end-time must be positive, at most 50 s, and an integer number of timesteps")
    if args.resume:
...
def transport_residual_roundoff(matrix, residual, row_scale):
    """Componentwise residual after only unavoidable float64 underflow.

    A row with n stored terms requires at most n multiplies, n additions,
    and one RHS subtraction. Each rounded operation can lose at most half
    the smallest subnormal. One whole subnormal per operation is used as
    a conservative absolute allowance. Normal-range relative tolerances
    are unchanged. This measures the residual; it never alters the solve.
    """
    absolute = np.abs(residual)
    row_terms = np.asarray(matrix.getnnz(axis=1), dtype=np.int64).ravel()
    allowance = (2 * row_terms + 1) * np.nextafter(np.float64(0), np.float64(1))
    resolved = np.maximum(absolute - allowance, 0.0)
    raw_relative = np.divide(absolute, row_scale,
                             out=np.where(absolute > 0, np.inf, 0.0),
                             where=row_scale > 0)
    relative = np.divide(resolved, row_scale,
                         out=np.where(resolved > 0, np.inf, 0.0),
                         where=row_scale > 0)
    return relative, raw_relative, allowance
...
class BalancedFacetTransport:
    ...
    def _advance(self, f_old, flux, dt, boundary_fresh, enforce_balance):
        ...
        residual = matrix @ new - rhs
        row_scale = abs(matrix) @ np.abs(new) + np.abs(rhs)
        relative, raw_relative, underflow_allowance = transport_residual_roundoff(
            matrix, residual, row_scale)
        linear_scaled = float(np.max(relative, initial=0.0))
        linear_scaled_raw = float(np.max(raw_relative, initial=0.0))
        ...
        metrics = {
            ...
            "linear_residual_scaled_raw": linear_scaled_raw,
            "linear_residual_underflow_allowance_max_m3": float(np.max(underflow_allowance)),
            "linear_residual_underflow_roundoff_only_rows": int(np.count_nonzero(
                (np.abs(residual) > 0) & (np.abs(residual) <= underflow_allowance))),
...
OUTPUT = ARGS.output
CASE_NAME = ARGS.case
CASE = {"refinement": {"coarse": 1, "fine": 2, "fine_half": 2,
                       "extra_fine": 4, "fine_quarter": 2, "extra_fine_half": 4, "extra_fine_quarter": 4}[CASE_NAME]}
STOP_REQUEST = {"requested": False, "signal": None}
def request_stop(signum, frame):
    STOP_REQUEST.update(requested=True, signal=int(signum))
    print("Stop requested; finishing the current accepted step before checkpointing.", flush=True)
for signum in (signal.SIGUSR1, signal.SIGTERM, signal.SIGINT):
    signal.signal(signum, request_stop)
state = {"status": "starting", "target_time_s": ARGS.end_time, "case": CASE_NAME}
...
try:
    ...
    dt_value = {"fine_half": 0.0005, "fine_quarter": 0.00025, "extra_fine_half": 0.0005, "extra_fine_quarter": 0.00025}.get(CASE_NAME, 0.001)
    T_final = ARGS.end_time
    num_steps = int(round(T_final/dt_value))
    checkpoint_steps = int(round(0.5/dt_value))
    log_steps = int(round(0.1/dt_value))
    comparison_steps = {int(round(t/dt_value)) for t in (0, 2, 5, 10, 20, 30, 40, 50)
                        if t <= T_final}
    comparison_steps.add(num_steps)
    ramp_time = 2.0
    ...
    def make_barycentric_mesh():
        ...
        # Coarse nodes are shown below, then each physical interval is halved.
        # 0--1 mm    : 0.5 mm spacing before halving
        ...
        # bottom 30 mm : 2 mm spacing before halving
    ...
    if ncells != 12546*CASE["refinement"]**2:
        raise RuntimeError("Unexpected mesh size for case " + CASE_NAME)
    ...
    config = {
        "case": CASE_NAME, "target_time_s": T_final, "dt_s": dt_value,
        ...
        "mesh_refinement": CASE["refinement"], "fresh_start": ARGS.resume is None,
        "resume_checkpoint": str(ARGS.resume) if ARGS.resume else None, "auto_restart": False,
        "checkpoint_interval_s": 0.5,
        "comparison_times_s": sorted(step*dt_value for step in comparison_steps),
        ...
        "coarse_flux_check_source_sha256": "a6a8efd740df37bfa67dc6329c616f4ee125aef1190484d22ef414a2a58b5462",
        "source_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "dolfin_version": dolfin.__version__, "numpy_version": np.__version__, "scipy_version": scipy.__version__,
        "claim_scope": "Matched mesh and timestep sensitivity through the configured target; accuracy assessed after comparison",
        "segment_stop_after_steps": ARGS.stop_after_steps,
        "fine_2s_parent_sha256": "b93f6ecf9245fda621ad3d8cd7d9a34ad0dc5bf413d087aa5aa1aa29c36be1b7",
        "matched_50s_parent_sha256": "594b3e9a83501bcde91bb59b9b5719d93b2bcef5f9a7a79e878f2f53dff0f4b1",
        "checkpoint_helper_sha256": hashlib.sha256(Path(__file__).with_name("study_checkpoint.py").read_bytes()).hexdigest(),
    }
    layout = dict(geometry_s_z=mesh.coordinates().copy(), topology=mesh.cells().copy(),
                  cell_dofs=cell_dofs, velocity_coordinates=velocity_coords,
                  velocity_ids=velocity_ids, pressure_coordinates=pressure_coords)
    identity = {key: config[key] for key in
                ("source_sha256", "checkpoint_helper_sha256", "case", "dt_s", "target_time_s",
                 "R_m", "H_m", "Q_cc_min", "rho_fresh_kg_m3", "rho_salt_kg_m3",
                 "nozzle_diameter_m", "ramp_time_s", "g_m_s2", "nu_m2_s",
                 "dolfin_version", "numpy_version", "scipy_version")}
    identity["mesh_and_dof_sha256"] = fingerprint(layout)
    ...
    def save_snapshot(name):
        ...
        with temp.open("wb") as handle:
            np.savez_compressed(handle,
                case=np.asarray(CASE_NAME), time_s=np.asarray(accepted_time), step=np.asarray(accepted_step),
    ...
    prior_columns = None
    if ARGS.resume:
        resume_identity = compatible_checkpoint_identity(ARGS.resume, identity)
        metadata, recovered = load_checkpoint(ARGS.resume, resume_identity, dt_value, num_steps,
                                              (V.dim(), P.dim(), F.dim()))
        config["restart_input_source_sha256"] = resume_identity["source_sha256"]
        config["audited_source_repair_restart"] = resume_identity != identity
        for function, key in ((U_n, "velocity_dofs"), (p_n, "pressure_dofs"),
                              (fresh_n, "fresh_dofs")):
            function.vector().set_local(recovered[key])
            function.vector().apply("insert")
        U_new.assign(U_n)
        U_star.assign(U_n)
        p_new.assign(p_n)
        accepted_step = metadata["step"]
        accepted_time = metadata["time_s"]
        attempted_step, attempted_time = accepted_step, accepted_time
        velocity_time = flux_time = accepted_time
        previous_fresh = fresh_n.vector().get_local()[cell_dofs].copy()
        candidate_fresh = previous_fresh.copy()
        shadow_fresh = np.full(ncells, np.nan)
        shadow_success = False
        raw_flux, sign_change_faces = facet_reader.read(U_n.vector().get_local())
        balanced_flux, unused_projection_metrics = transport.balance(raw_flux)
        flow_stages = {key: {"time_s": accepted_time, "status": "complete"}
                       for key in flow_stages}
        cumulative_in = metadata["cumulative_in"]
        cumulative_out = metadata["cumulative_out"]
        max_budget_relative = metadata["max_budget_relative"]
        max_flux_change_relative = metadata["max_flux_change_relative"]
        last_row = metadata["last_row"]
        segments = metadata["segments"]
        mass = float(np.dot(transport.cell_volumes, previous_fresh))
        if abs(mass-(cumulative_in-cumulative_out)) > 1e-8*max(cumulative_in, 1e-30):
            raise ValueError("Checkpoint inventory does not match cumulative source/outflow")
        prior_columns = carry_budget(ARGS.resume.parent/"verification_budget.csv",
                                     OUTPUT/"verification_budget.csv", metadata, dt_value)
        for step in sorted(comparison_steps):
            if step <= accepted_step:
                filename = "field_step_{:09d}.npz".format(step)
                source = ARGS.resume.parent/filename
                # Required matched fields travel alongside each return checkpoint.
                with np.load(str(source), allow_pickle=False) as data:
                    if (str(data["case"].item()) != CASE_NAME or int(data["step"]) != step
                            or float(data["dt_s"]) != dt_value
                            or str(data["source_sha256"].item()) not in compatible_history_sources(identity)
                            or str(data["mesh_and_dof_sha256"].item()) != identity["mesh_and_dof_sha256"]):
                        raise ValueError("Mismatched historical comparison field: " + filename)
                shutil.copy2(str(source), str(OUTPUT/filename))
    initial_step = accepted_step
    segments.append({"start_step": initial_step, "start_time_s": accepted_time,
                     "slurm_job_id": os.environ.get("SLURM_JOB_ID"),
                     "source_sha256": identity["source_sha256"],
                     "restart_input_source_sha256": config.get("restart_input_source_sha256"),
                     "output_directory": str(OUTPUT)})
    config.update(initial_step=initial_step, initial_time_s=accepted_time, segments=segments,
                  initial_cumulative_in_m3=cumulative_in, initial_cumulative_out_m3=cumulative_out)
    ...
    def save_checkpoint():
        ...
        write_checkpoint(OUTPUT/"checkpoint_latest.npz", identity, accepted_step,
                         accepted_time, dt_value, U_n.vector().get_local(),
                         p_n.vector().get_local(), fresh_n.vector().get_local(),
                         cumulative_in, cumulative_out, max_budget_relative,
                         max_flux_change_relative, clean_json(last_row), segments)

    def save_comparison():
        atomic_npz(OUTPUT/"field_step_{:09d}.npz".format(accepted_step),
                   case=np.asarray(CASE_NAME), time_s=np.asarray(accepted_time),
                   step=np.asarray(accepted_step), dt_s=np.asarray(dt_value),
                   source_sha256=np.asarray(identity["source_sha256"]),
                   mesh_and_dof_sha256=np.asarray(identity["mesh_and_dof_sha256"]),
                   geometry_s_z=mesh.coordinates().copy(), topology=mesh.cells().copy(),
                   cell_volumes_m3=transport.cell_volumes,
                   fresh=fresh_n.vector().get_local()[cell_dofs])

    budget_handle = (OUTPUT/"verification_budget.csv").open("a", newline="", buffering=1)
    budget_writer = csv.DictWriter(budget_handle, fieldnames=prior_columns) if prior_columns else None
    snapshot_ready = True
    if not ARGS.resume:
        save_comparison()
    save_checkpoint()
    state.update(status="running", last_recorded_step=accepted_step,
                 last_recorded_time_s=accepted_time, initial_step=initial_step)
    write_json(OUTPUT, "verification_status.json", state)
    print("Case={}, starting at step {}, target={} s, dt={} s, total steps={}. Output: {}".format(
          CASE_NAME, initial_step, T_final, dt_value, num_steps, OUTPUT), flush=True)
    for n_step in range(initial_step+1, num_steps+1):
        if STOP_REQUEST["requested"]:
            paused = True
            break
        step_wall_start = wallclock.time()
        ...
        last_row = {
            ...
            "step_wall_s": wallclock.time()-step_wall_start,
            "segment_initial_step": initial_step,
            "fresh_centroid_z_m": float(np.dot(transport.cell_volumes*candidate_fresh, transport.centroids[:, 1])/mass) if mass > 0 else 0.0,
        }
        ...
        if n_step <= initial_step+5 or n_step % log_steps == 0:
            budget_handle.flush()
            ...
            write_json(OUTPUT, "verification_status.json", state)
        if n_step in comparison_steps:
            save_comparison()
        pause_due = (STOP_REQUEST["requested"] or
                     (ARGS.stop_after_steps > 0 and n_step-initial_step >= ARGS.stop_after_steps))
        if n_step % checkpoint_steps == 0 or pause_due or n_step == num_steps:
            save_checkpoint()
        if pause_due and n_step < num_steps:
            paused = True
            break

    # Only an accepted checkpoint is restartable; attempted failure snapshots are not.
    save_checkpoint()
    save_snapshot("verification_paused.npz" if paused else "verification_final.npz")
    budget_handle.flush()
    budget_handle.close()
    state.update(status="paused" if paused else "completed", last_recorded_step=accepted_step,
                 last_recorded_time_s=accepted_time, stop_signal=STOP_REQUEST["signal"],
                 wall_elapsed_s=wallclock.time()-wall_start)
    write_json(OUTPUT, "verification_status.json", state)
    print("Paused safely; resume from the RETURN folder." if paused else
          "Completed; launcher will assemble the RETURN folder and ZIP.", flush=True)
