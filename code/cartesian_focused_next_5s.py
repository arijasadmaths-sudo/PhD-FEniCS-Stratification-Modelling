#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Next short Cartesian mesh check: 0.3125 mm core, 388 x 254 FV cells.

Default case focus312p5 starts from rest and reaches exactly t=5 s at dt=.001 s.
Compare with the completed focus625 case (0.625 mm, 252 x 154 FV cells).
The earlier focus1250 and focus625 cases remain available for reproducibility.

Fine core x=[.26,.34] m, z=[0,.06] m; full domain [.0,.6] x [.0,.3] m.
Outer spacing grows smoothly to <=5 mm; the 10 mm inlet has 32 aligned faces.
The physical parameters and Oseen-IPCS equations are unchanged from the
completed focused runs. The scalar transport remains implicit conservative
first-order upwind with D=1e-7 m^2/s, physical cell areas/transmissibilities,
and interior-only balancing of face fluxes. No clipping or mass rescaling.

Legacy DOLFIN 2019.1, one MPI rank:
  python3 cartesian_focused_next_5s.py --case focus312p5 --output NEW_DIRECTORY
  python3 cartesian_focused_next_5s.py --self-test
  python3 cartesian_focused_next_5s.py --describe
...
Do not use a focus625 or focus1250 checkpoint to start the new mesh.

This is a short spatial-sensitivity check, not a claim of late-time accuracy.
"""
import argparse
import csv
import datetime
...
CASE_H = {'focus1250':.00125, 'focus625':.000625, 'focus312p5':.0003125}
...
def run_case(args):
    ...
    try:
        ...
        expected_shape={'focus1250':(178,101),'focus625':(252,154),'focus312p5':(388,254)}[args.case]
        ...
        config={'method':'Oseen IPCS + conservative FV on graded rectangular cells',
            ...
            'interpretation':'Compare focus312p5 with completed focus625 at 5 s; focus1250 already matches uniform480 at 5 s. No late-time or global mesh-independence claim'}
...
class KernelTests(unittest.TestCase):
    def test_grid_domain_nozzle_alignment_and_grading(self):
        for case,h in CASE_H.items():
            ...
            self.assertEqual((len(x)-1,len(y)-1),{'focus1250':(178,101),'focus625':(252,154),'focus312p5':(388,254)}[case])
    ...
    def test_atomic_checkpoint_restart_and_wrong_case_rejection(self):
        ...
        data=dict(metadata_json=json.dumps({'format_version':1,'case':'focus312p5','fingerprint':'test'}),
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            step,found=validate_restart(loaded,'focus312p5','test',g,vel,pre,mesh_xy,mesh_cells)
            ...
            with self.assertRaises(ValueError): validate_restart(loaded,'focus312p5','changed',g,vel,pre,mesh_xy,mesh_cells)
            loaded['mesh_cells']=loaded['mesh_cells'][::-1]
            with self.assertRaises(ValueError): validate_restart(loaded,'focus312p5','test',g,vel,pre,mesh_xy,mesh_cells)
            self.assertEqual(os.listdir(folder),['checkpoint_latest.npz'])

def main():
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--case',choices=tuple(CASE_H),default='focus312p5')
