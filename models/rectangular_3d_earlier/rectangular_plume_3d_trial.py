...
# RECTANGULAR PLUME -- FULL 3D TANK, SEPARATE TRIAL RUN
# Legacy FEniCS / DOLFIN 2019.1; MPI parallel solver

# Run with submit_rectangular_plume_3d_trial.sh.
# Full box: 0.800 x 0.400 x 0.450 m. No quarter/symmetry boundaries.
# These dimensions are retained from the proposed larger-tank branch;
# confirm the fluid-domain dimensions before quantitative experiment comparison.

# MODEL LIMITATIONS:
# * The 2 mm nozzle is a flush circular inlet, not a resolved pipe/lip.
# * Outflow is distributed over the floor outside the nozzle, NOT a measured
#   experimental outlet. The ceiling and side walls are rigid/no-slip.
# * Scalar diffusivity D=1e-7 m^2/s is the inherited modelling value, not a
#   verified molecular salt diffusivity. DG0 transport adds numerical diffusion.
# * Syntax checked only: no new DOLFIN/MPI run or convergence study performed.

# Retains Taylor-Hood P2/P1 Oseen flow, Boussinesq buoyancy, H(div) flux
# reconstruction, conservative implicit DG0 upwind scalar transport.
# Default: Q=20 cc/min, dt=0.001 s, target=20 s, up to 0.5 s per submission.
# Re-submit the same launcher to resume this trial's checkpoint.
# A distinct results root prevents accidental reuse of the previous 3D run.
# Optional RECT3D_OUTPUT_ROOT selects a different trial directory explicitly.
# Checkpoints: every 100 steps or 600 wall seconds (between completed steps).
# Soft wall budget: 5400 seconds including setup, checked between steps;
# allows time to save before the launcher's two-hour scheduler limit.
# This is best effort: a single solve/save can exceed the reserved time.
...
# Cadences are in timesteps: 0.1 s / 0.25 s at the default dt=0.001 s.
diagnostic_every = 100
write_every = 250

# Full restart checkpoint every 100 timesteps.
...
checkpoint_wall_seconds = float(
    os.environ.get("CHECKPOINT_WALL_SECONDS", "600.0")
)

wall_time_budget_seconds = float(
    os.environ.get("WALL_TIME_BUDGET_SECONDS", "5400.0")
)
...
default_output_root = (
    "results_rectangular_plume_3d_trial_Q{}ccmin_eps{}_dt{}".format(
        flow_tag,
        perturbation_tag,
        dt_tag,
    )
...
solver_schema = "rectangular_plume_3d_trial_schema_20260904a"
mesh_recipe = (
...
def write_configuration_and_diagnostics_header():
    ...
    if rank == 0:
        try:
            configuration = {
                "model": "full rectangular trial with distributed floor return",
...
for step in range(start_step + 1, end_step + 1):
    ...
    if rank == 0:
        stop_for_wall_budget = (
            wall_time_budget_seconds > 0.0
            and time.monotonic() - run_wall_start >= wall_time_budget_seconds
        )
    else:
        stop_for_wall_budget = None
    stop_for_wall_budget = pycomm.bcast(stop_for_wall_budget, root=0)
    ...
    if stop_for_wall_budget:
        root_print("Soft wall budget reached; accepted state saved. Re-submit to resume.")
        break
