#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Five-second Cartesian plume pilot: conservative scalar transport, no clipping.

Run with legacy FEniCS/DOLFIN 2019.1, NumPy, SciPy and mpi4py, ONE MPI rank:
    python3 cartesian_ipcs_conservative_5s.py --output NEW_EMPTY_DIRECTORY
Self-tests (NumPy/SciPy only):
    python3 cartesian_ipcs_conservative_5s.py --self-test

Preserved: 0.60 x 0.30 m domain; original P2/P1 Oseen IPCS equations; velocity
mesh 120 x 60 subdivisions; 0.01 m parabolic slot at Umax=0.001 m/s;
flux-balanced broad returns; nu=1e-6; scalar D=1e-7 m2/s; beta=-0.00627;
dt=0.001 s; half-cosine 0.5 s velocity ramp; initially c=1, nozzle c=0.

CHANGED SCALAR METHOD: one salt-fraction average per rectangular cell,
implicit first-order upwind advection, orthogonal two-point diffusion.
P2 velocity face integrals are corrected on INTERNAL faces to local volume
balance. External prescribed fluxes are preserved. Correction magnitude is
logged; no claim of an unchanged transport velocity or converged accuracy.
There is NO clipping, scalar filtering, or global scalar-mass adjustment.

Finite-volume values feed buoyancy through DG0 on the two triangles in each
rectangle. c.xdmf/c.h5 therefore contain CELL data, not the old CG1 vertex
data. Use fv_step_*.npz (c, x_m, y_m, time_s) for rectangular profiles.
Scalar c is raw salt fraction: c=1 ambient and c=0 fresh source. Outputs are
NOT min-max normalised. Saved transport_fx/fy are integrated volume fluxes
[m2/s]; divide by face length for the transport normal velocity.

The scalar budget includes the actual numerical advective inlet/outlet and
nozzle diffusion. Diffusion at other boundaries is zero. A successful pilot
checks the new numerical scheme; mesh/time convergence and the effect of
first-order numerical diffusion still need assessment before thesis claims.
No complete coupled FEniCS run was possible in the authoring environment.
The pure scalar/flux tests are embedded and the launch script runs them first.
"""
import argparse
import csv
import datetime
...
def run_pilot(args):
    ...
    write_status()
    try:
        # Same physical parameters and velocity mesh as IPSC_0.001.py.
        Lx, Ly, nx, ny = 0.60, 0.30, 120, 60
        hx, hy = Lx / nx, Ly / ny
        d_nozzle, U_in = 0.01, 0.001
        xL, xR = (Lx-d_nozzle)/2.0, (Lx+d_nozzle)/2.0
        U_return = U_in * d_nozzle / (2.0*xL)
        dt_value, ramp_time, D_value = 0.001, 0.5, 1e-7
        write_every, log_every = 250, 100
        ...
        c = np.ones((ny,nx))
        set_scalar(c)
        nozzle = ((x_edges[:-1] >= xL-tol) & (x_edges[1:] <= xR+tol))
        if np.count_nonzero(nozzle) != 2 or abs(np.sum(nozzle)*hx-d_nozzle) > 1e-12:
            raise RuntimeError("Nozzle must cover exactly two complete FV bottom faces")
        ...
        config = {"method":"Oseen IPCS + conservative rectangular FV scalar",
                  ...
                  "interpretation":"Five-second numerical pilot, not mesh-converged physical validation. First-order upwind introduces numerical diffusion.",
        ...
        for field in ("u","p","c"):
            ...
            outputs.append(out)
        def save_fields(step, time, fx=None, fy=None):
...
def main():
    ...
    parser.add_argument("--output",default=os.environ.get("CARTESIAN_FV_OUTPUT",
                       "cartesian_conservative_5s_"+datetime.datetime.now().strftime("%Y%m%d_%H%M%S")))
    parser.add_argument("--steps",type=int,default=5000,help="Pilot steps, 1 to 5000; dt=0.001 s")
    ...
    if not 1 <= args.steps <= 5000:
        parser.error("This diagnostic pilot allows 1 to 5000 steps")
    run_pilot(args)
