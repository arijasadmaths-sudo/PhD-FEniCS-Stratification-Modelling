#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Cartesian compact meshes at half the original time step.

Both cases start from rest: dt=0.0005 s, 10,000 steps, final time=5 s.
Meshes, physical parameters, FE equations, flux projection and scalar scheme
are unchanged from the returned dt=0.001 s compact solver. This pair completes
the mesh-by-time-step comparison; it does not establish late-time convergence.

Use the supplied Slurm launcher with one MPI rank and DOLFIN 2019.1.
A resumed allocation must use this exact source, case and dt=0.0005 checkpoint.
Old dt=0.001 checkpoints are rejected. Each allocation writes a new segment.
Fields/checkpoints remain every 0.25 simulated seconds; log sampling every 0.1 s.
SIGUSR1/SIGTERM requests a checkpoint at the next accepted state.
"""
import argparse
import csv
import datetime
...
CASE_H = {'compact312p5':.0003125, 'compact156p25':.00015625}
PARENT_FOCUS312P5_SHA256='4bfbde0c5b7f53c030929818fab687a2e9468c4660db94c01622dac8d5f65128'
PREVIOUS_FOCUSED_SHA256='ed7638fa2ccff3f991b251d9d1ed1ff9c13a65a800db48c090654b1ab8fa5a6c'
...
def make_graded_grid(core_h, Lx=0.60, Ly=0.30,
                     core_x=(0.28, 0.32), core_z_top=0.04,
...
class BalancedFluxGrid(object):
    ...
    def balance(self, fx, fy):
        ...
        # Refine the same graph solve when accumulated factorization roundoff
        # leaves a measurable residual on the finer grid. Exterior fluxes and
        # the projection/discretisation are unchanged. Reuse the factorization.
        for refinement in range(2):
            if self.n <= 1 or np.max(np.abs(after)) <= 1.e-13 * max(scale, self._flux_scale(new_fx, new_fy)):
                break
            increment = np.zeros(self.n)
            increment[1:] = self._project_solve(after[1:])
            delta = self.weights * (increment[self.u] - increment[self.v])
            new_fx[:, 1:-1] -= delta[:self.n_xinterior].reshape(self.ny, self.nx - 1)
            new_fy[1:-1, :] -= delta[self.n_xinterior:].reshape(self.ny - 1, self.nx)
            correction += delta
            after = self.divergence(new_fx, new_fy).ravel()
...
def validate_restart(a,case,fingerprint,grid,velocity_xy,pressure_xy,mesh_xy,mesh_cells):
    ...
    if meta.get('dt_s') != DT_S or meta.get('source_sha256') != source_hash():
        raise ValueError('Checkpoint source/time step mismatch; dt=0.001 checkpoints cannot resume here')
    ...
    step=int(a['step'])
    if step<0 or step>=TOTAL_STEPS or abs(float(a['time_s'])-DT_S*step)>1e-12:
...
def run_case(args):
    ...
    status={'status':'starting','verification_case':args.case,'requested_steps':TOTAL_STEPS,
    ...
    try:
        ...
        expected_shape={'compact312p5':(268,194),'compact156p25':(406,327)}[args.case]
        if (nx,ny)!=expected_shape: raise RuntimeError('Unexpected grid shape')
        dt_value=DT_S; D_value=1e-7; ramp_time=.5
        ...
        fingerprint=hashlib.sha256((source_hash()+'|'+args.case+'|dt=.0005|T=5').encode()).hexdigest()
        ...
        config={'method':'Oseen IPCS + conservative FV on graded rectangular cells',
            'verification_case':args.case,'baseline_uniform480_sha256':BASELINE_SHA256,'previous_focused_sha256':PREVIOUS_FOCUSED_SHA256,
            'parent_focus312p5_sha256':PARENT_FOCUS312P5_SHA256,
            ...
            'core_spacing_m':CASE_H[args.case],'core_x_m':[.28,.32],'core_z_m':[0.,.04],
            ...
            'ambient_c':1.,'inlet_c':0.,'dt_s':dt_value,'requested_steps':TOTAL_STEPS,'requested_duration_s':5.,
            ...
            'interpretation':'Compare each compact mesh with its dt=0.001 run, then compare meshes at matching dt=0.0005. Fresh starts with matched physical output times; no late-time or global mesh-independence claim'}
        ...
        def save_fields(step,fx,fy,rawfx,rawfy):
            ...
            if step==TOTAL_STEPS: atomic_npz(os.path.join(folder,'fv_final.npz'),**data)
        def checkpoint_impl(step):
            # The checkpoint must not outrun its available accepted CSV rows.
            if budget_file is not None:
                budget_file.flush()
                os.fsync(budget_file.fileno())
        ...
        print('Case {}: {} x {} FV cells, {} triangles; dt=.0005s, target5s'.format(args.case,nx,ny,mesh.num_cells()),flush=True)
        ...
        for step in range(start_step+1,TOTAL_STEPS+1):
            ...
            sampled=step<=10 or step%LOG_STEPS==0 or step==TOTAL_STEPS
            ...
            if sampled:
                ...
                print('Step {}/10000 t={:.3f}s c=[{:.9g},{:.9g}] Vf={:.9g} R={:.3e} elapsed={:.1f}min'.format(
                    step,t,c.min(),c.max(),inventory,residual,(clock.perf_counter()-wall_start)/60),flush=True)
            if step==1 or step%FIELD_STEPS==0 or step==TOTAL_STEPS or stop_requested[0]:
                tick=clock.perf_counter()
...
class KernelTests(unittest.TestCase):
    def test_grid_domain_nozzle_alignment_and_grading(self):
        for case,h in CASE_H.items():
            ...
            self.assertEqual((len(x)-1,len(y)-1),{'compact312p5':(268,194),'compact156p25':(406,327)}[case])
    ...
    def test_all_full_grids_projection_constant_bounds_and_source_budget(self):
        for case,h in CASE_H.items():
    ...
    def test_atomic_checkpoint_restart_and_wrong_case_rejection(self):
        ...
        data=dict(metadata_json=json.dumps({'format_version':1,'case':'compact312p5','fingerprint':'test','dt_s':DT_S,'source_sha256':source_hash()}),
            ...
            step=10,time_s=10*DT_S,cumulative_budget_m2=cum,vf_m2=cum[0],cumulative_wall_s=1.)
        with tempfile.TemporaryDirectory() as folder:
            ...
            step,found=validate_restart(loaded,'compact312p5','test',g,vel,pre,mesh_xy,mesh_cells)
            self.assertEqual(step,10); np.testing.assert_array_equal(found,cum)
            with self.assertRaises(ValueError): validate_restart(loaded,'compact156p25','test',g,vel,pre,mesh_xy,mesh_cells)
            with self.assertRaises(ValueError): validate_restart(loaded,'compact312p5','changed',g,vel,pre,mesh_xy,mesh_cells)
            loaded['mesh_cells']=loaded['mesh_cells'][::-1]
            with self.assertRaises(ValueError): validate_restart(loaded,'compact312p5','test',g,vel,pre,mesh_xy,mesh_cells)
            self.assertEqual(os.listdir(folder),['checkpoint_latest.npz'])

def main():
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--case',choices=tuple(CASE_H),default='compact312p5')
    ...
    if args.describe:
        for case,h in CASE_H.items():
            x,y=make_graded_grid(h)
            print(json.dumps({'case':case,'nx':len(x)-1,'ny':len(y)-1,'core_spacing_m':h,'dt_s':DT_S,'T_s':FINAL_TIME_S,'steps':TOTAL_STEPS}))
        return 0
