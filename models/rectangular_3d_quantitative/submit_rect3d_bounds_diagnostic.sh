#!/bin/bash -l
#SBATCH --job-name=rect3d_bounds_diag
#SBATCH --nodes=1
#SBATCH --ntasks=24
...
if [[ $# -ne 0 ]]; then
    echo "Use bash submit_rect3d_bounds_diagnostic.sh with no arguments." >&2
    exit 2
fi
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    bounds_code_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
    for bounds_required in diagnose_rect3d_scalar_bounds.py rect3d_bounds_report.py mixed_scalar_transport.py verify_mixed_scalar_transport.py; do
    ...
    exec sbatch --account=YOUR_ACCOUNT --partition=compute --chdir="${bounds_code_dir}" "${bounds_code_dir}/submit_rect3d_bounds_diagnostic.sh"
fi
...
export RECT3D_VERIFY_ONLY=0 MESH_LEVEL=fine DT=0.001 T_GLOBAL=0.05 SEGMENT_DURATION=0.05
...
echo "Isolated fine-mesh bounds diagnostic: job ${SLURM_JOB_ID}; 50 steps; dt=0.001; maximum 30 minutes."
...
diagnostic_status=0
srun "${srun_options[@]}" "${FENICS_PYTHON}" -u "${run_dir}/diagnose_rect3d_scalar_bounds.py" || diagnostic_status=$?
