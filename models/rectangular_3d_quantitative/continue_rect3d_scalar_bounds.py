# ISOLATED DIAGNOSTIC CONTINUATION: 50 further steps, from 0.050 to 0.100 s.
# Uses the production weak forms; raises the diagnostic-only emergency bound to 2%.
# Checkpoints carry a separate schema and results go to a unique diagnostic root.
...
from rect3d_bounds_followup_report import FollowupBoundsReport as BoundsReport, prepare_diagnostic_checkpoint

if SCALAR_SOLVER_REVISION != "equilibrated-2026-09-17":
    raise RuntimeError("Use the equilibrated-2026-09-17 mixed_scalar_transport.py for this diagnostic.")
diagnostic_job_tag = os.environ.get("SLURM_JOB_ID", "")
if not diagnostic_job_tag.isdigit():
    raise RuntimeError("Submit this bounded diagnostic through its Slurm launcher.")
# Explicitly bound this separate diagnostic, including when inherited shell
# variables refer to another study. These are the failed fine case's parameters.
for _key, _value in {
    "MESH_LEVEL": "fine", "FLOW_RATE_CC_MIN": "20", "DT": "0.001",
    "T_GLOBAL": "0.1", "SEGMENT_DURATION": "0.05",
    "SCALAR_DIFFUSIVITY": "1.5e-9", "DYE_DIFFUSIVITY": "4.14e-10",
    "INLET_PERTURBATION": "0", "PIN_CHECKPOINT_TIME": "0", "SEED_CHECKPOINT": "",
    "CHECKPOINT_WALL_SECONDS": "600", "WALL_TIME_BUDGET_SECONDS": "1440",
    "SAVE_MARGIN_SECONDS": "180",
}.items():
...
requested_output_root = "diagnostic_rect3d_bounds_followup_" + diagnostic_job_tag
diagnostic_source_checkpoint = os.path.join(
    script_directory, "diagnostic_rect3d_bounds_18989285", "checkpoint", "state_step_000000050.npz")
diagnostic_restart_origin = None
...
scalar_abort_margin = 0.02  # Diagnostic emergency ceiling only; production remains 0.001.
...
solver_schema = "rect3d_bounds_diagnostic_only_20260917a"
...
if rank == 0:
    try:
        diagnostic_restart_origin = prepare_diagnostic_checkpoint(
            diagnostic_source_checkpoint, CHECKPOINT_DIR,
            _read_npz_metadata, validate_checkpoint_metadata)
        root_print("DIAGNOSTIC CONTINUATION: copied step-50 source into a new diagnostic directory.")
        root_print("Source checkpoint:", diagnostic_source_checkpoint)
...
diagnostic_restart_origin = pycomm.bcast(diagnostic_restart_origin, root=0)
...
def write_configuration_and_diagnostics_header():
    ...
    if rank == 0:
        try:
            configuration = {
                "model": "Scalar bounds startup diagnostic; not production results",
                "production_eligible": False,
                "production_scalar_margin": 0.001,
                "diagnostic_emergency_margin": 0.02,
...
if start_step != 50 or abs(start_time - 0.05) > 1e-14:
    raise RuntimeError("Diagnostic continuation must resume the saved step-50 state; a clean start is forbidden.")
...
root_print("")
root_print("================================================")
root_print("STARTING DIAGNOSTIC CONTINUATION: 0.050 TO 0.100 SECONDS")
root_print("================================================")
root_print("dt                 =", dt)
...
bounds_report = BoundsReport(globals())
root_print("BOUNDS DIAGNOSTIC: at most 50 timesteps; production margin 0.001; emergency margin 0.02.")
...
for step in range(start_step + 1, end_step + 1):
    ...
    if (
        ...
        bounds_report.report["hard_abort"] = dict(
            step=int(step), time_s=float(t_now),
            velocity_component_linf=(float(velocity_component_linf) if math.isfinite(velocity_component_linf) else None),
            density_min=(float(raw_c_min) if math.isfinite(raw_c_min) else None),
            density_max=(float(raw_c_max) if math.isfinite(raw_c_max) else None),
            dye_min=(float(raw_dye_min) if math.isfinite(raw_dye_min) else None),
            dye_max=(float(raw_dye_max) if math.isfinite(raw_dye_max) else None),
        )
        bounds_report.finish("diagnostic emergency ceiling or nonfinite field")
        raise RuntimeError(
            "Safety abort at step {} t={}: |u|inf={}, c=[{},{}], dye=[{},{}]".format(
                step,
                t_now,
    ...
    bounds_report.observe()

    
    # ADVANCE THE DIAGNOSTIC ONLY (not eligible as production results)
...
bounds_report.finish(stop_reason if stop_for_wall_budget else "50-step diagnostic horizon")
...
root_print("")
root_print("================================================")
root_print("ISOLATED SCALAR BOUNDS DIAGNOSTIC FINISHED; NOT PRODUCTION RESULTS")
root_print("================================================")
...
if completed_step < max_global_step:
    root_print("")
    root_print("Diagnostic stopped early. Return its report; this job has no continuation chain.")
else:
    root_print("")
    root_print("DIAGNOSTIC CONTINUATION REACHED 0.100 SECONDS.")
