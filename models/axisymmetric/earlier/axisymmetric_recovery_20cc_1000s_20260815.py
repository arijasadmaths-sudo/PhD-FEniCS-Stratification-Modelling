...
# Original cancelled run.  This folder contains the old visualisation
# HDF5 files with a complete snapshot at t = 650 s.
LEGACY_SOURCE_FOLDER = (
    "/path/to/work/output_axisym_massfix_dt001_1000s"
)

# IMPORTANT: use a NEW folder for the recovered continuation.
# This prevents any checkpoints made by the accidentally started
# restart-from-zero job from being detected.
OUTPUT_FOLDER = (
    "/path/to/work/"
    "output_axisym_massfix_dt001_1000s_recovered650"
)

dt_value = 0.001
num_steps = 1000000          # absolute final target: t = 1000 s
write_every = 50000          # visualisation every 50 s
diagnostic_every = 5000      # diagnostics every 5 s
ramp_time = 0.5

# The cancelled original run wrote visualisation output at
# t = 0, 50, ..., 650 s.  Index 13 is therefore the t = 650 s state.
LEGACY_RECOVERY_INDEX = 13
LEGACY_RECOVERY_STEP = 650000

# Proper restart/checkpoint controls for all future jobs.
AUTO_RESTART = True
checkpoint_every = 10000     # full-state checkpoint every 10 s
KEEP_CHECKPOINTS = 2
...
# Priority:
#   1. If this recovered continuation has already produced a proper
#      checkpoint, restart exactly from the newest one.
#   2. Otherwise recover the old cancelled calculation from the
#      visualisation HDF5 snapshot at t = 650 s.

# The old pressure (CG1) and fresh scalar (DG0) are recovered exactly
# from their saved nodal/cell values.  The old transformed velocity
# was CG2, but ordinary XDMF visualisation stored only mesh-vertex
# values.  It is therefore reconstructed as a CG1 nodal field and
# interpolated back into CG2.  This introduces a small one-time
# restart perturbation in the internal quadratic velocity DOFs.
...
def cumulative_discrete_source_in(step):
    """
    Reproduce the source-volume integration used by the original
    time loop.  The inlet rate is Q_nozzle_mesh times the start-up
    ramp, evaluated at the end of each completed time step.
    """

    if step <= 0:
        return 0.0

    # Only the first ~0.5 s needs explicit ramp evaluation.
    n_start = min(
        int(step),
        int(np.ceil(ramp_time / dt_value)) + 2
    )
...
def _coordinate_key(point):
    # Coordinates in the old and regenerated meshes are deterministic,
    # but rounding makes the recovery check insensitive to tiny HDF5
...
def recover_legacy_visualisation_state():
    """
    Recover step 650000 from the ORIGINAL cancelled run.
    ...
    All three contain VisualisationVector/13 at t = 650 s.
    """

    if h5py is None:
        raise RuntimeError(
            "The one-time 650 s recovery needs the Python package h5py. "
            "On BluePebble, first try: python -c 'import h5py'."
        )
    ...
    for path in (U_file, p_file, fresh_file):
        if not os.path.exists(path):
            raise RuntimeError(
                "Legacy recovery file not found: " + path
            )

    dataset_name = (
        "/VisualisationVector/"
        + str(LEGACY_RECOVERY_INDEX)
    )

    with h5py.File(U_file, "r") as h5_U:
        source_geometry = np.asarray(
            h5_U["/Mesh/0/mesh/geometry"][:],
            dtype=float
        )

        source_topology = np.asarray(
            h5_U["/Mesh/0/mesh/topology"][:],
            dtype=np.int64
        )

        U_vertex_source = np.asarray(
            h5_U[dataset_name][:],
            dtype=float
        )

    with h5py.File(p_file, "r") as h5_p:
        p_vertex_source = np.asarray(
            h5_p[dataset_name][:, 0],
            dtype=float
        )

    with h5py.File(fresh_file, "r") as h5_f:
        fresh_cell_source = np.asarray(
            h5_f[dataset_name][:, 0],
            dtype=float
        )
    ...
    current_topology = np.asarray(
        mesh.cells(),
        dtype=np.int64
    )

    if source_geometry.shape != current_geometry.shape:
        raise RuntimeError(
            "Legacy and regenerated meshes have different "
            "vertex-array shapes."
        )

    if source_topology.shape != current_topology.shape:
        raise RuntimeError(
            "Legacy and regenerated meshes have different "
            "cell-array shapes."
        )

    if U_vertex_source.shape[0] != source_geometry.shape[0]:
        raise RuntimeError(
            "Legacy velocity vertex count is inconsistent."
        )

    if p_vertex_source.shape[0] != source_geometry.shape[0]:
        raise RuntimeError(
            "Legacy pressure vertex count is inconsistent."
        )

    if fresh_cell_source.shape[0] != source_topology.shape[0]:
        raise RuntimeError(
            "Legacy DG0 cell count is inconsistent."
        )

    
    # Map old vertex numbering to the regenerated deterministic mesh.
    
    current_vertex_from_coord = {}

    for current_vertex, point in enumerate(current_geometry):
        key = _coordinate_key(point)

        if key in current_vertex_from_coord:
            raise RuntimeError(
                "Duplicate coordinate key while rebuilding old mesh."
            )

        current_vertex_from_coord[key] = current_vertex

    source_to_current_vertex = np.empty(
        source_geometry.shape[0],
        dtype=np.int64
    )

    for source_vertex, point in enumerate(source_geometry):
        key = _coordinate_key(point)

        if key not in current_vertex_from_coord:
            raise RuntimeError(
                "A legacy mesh vertex is absent from the regenerated mesh."
            )

        source_to_current_vertex[source_vertex] = (
            current_vertex_from_coord[key]
        )

    
    # Pressure: old output is CG1 nodal data, so this recovery is exact.
    
    p_at_current_vertices = np.empty(
        mesh.num_vertices(),
        dtype=float
    )

    for source_vertex, current_vertex in enumerate(
        source_to_current_vertex
    ):
        p_at_current_vertices[current_vertex] = (
            p_vertex_source[source_vertex]
        )

    p_v2d = vertex_to_dof_map(P)
    ...
    p_values[p_v2d] = p_at_current_vertices

    p_n.vector().set_local(p_values)
    p_n.vector().apply("insert")

    
    # Fresh scalar: old output is DG0 cell-centred data.
    # Map cells by their vertex sets, so recovery does not depend
    # on cell ordering.
    
    current_cell_from_vertices = {}

    for current_cell_index, tri in enumerate(current_topology):
        key = tuple(
            sorted(
                int(v)
                for v in tri
        ...
        current_cell_from_vertices[key] = current_cell_index

    fresh_values = np.zeros(
        F.dim(),
        dtype=float
    )

    for source_cell_index, source_tri in enumerate(source_topology):
        ...
        mapped_vertices = [
            ...
            for source_vertex in source_tri
        ]

        cell_key = tuple(
            sorted(mapped_vertices)
        )

        if cell_key not in current_cell_from_vertices:
            raise RuntimeError(
                "A legacy cell is absent from the regenerated mesh."
            )

        current_cell_index = (
            current_cell_from_vertices[cell_key]
        )
        ...
        fresh_values[scalar_dof] = (
            fresh_cell_source[source_cell_index]
        )

    fresh_n.vector().set_local(fresh_values)
    fresh_n.vector().apply("insert")

    
    # Transformed velocity:
    
    # The old CG2 Function was written with ordinary XDMF
    # visualisation output, which contains values only at mesh
    # vertices.  Reconstruct the exact saved nodal field in vector
    # CG1, then interpolate it to the production CG2 space.
    
    if U_vertex_source.shape[1] < 2:
        raise RuntimeError(
            "Legacy transformed velocity does not contain two components."
        )

    U_at_current_vertices = np.empty(
        (mesh.num_vertices(), 2),
        dtype=float
    )
    ...
    for source_vertex, current_vertex in enumerate(
        ...
        U_at_current_vertices[current_vertex, 0] = (
            U_vertex_source[source_vertex, 0]
        )

        U_at_current_vertices[current_vertex, 1] = (
            U_vertex_source[source_vertex, 1]
        )

    P1_recovery = FunctionSpace(
        mesh,
        "CG",
        1
    )

    V1_recovery = VectorFunctionSpace(
        mesh,
        "CG",
        1
    )

    U_s_CG1 = Function(P1_recovery)
    U_z_CG1 = Function(P1_recovery)

    rec_v2d = vertex_to_dof_map(
        P1_recovery
    )
    ...
    U_s_values[rec_v2d] = (
        U_at_current_vertices[:, 0]
    )

    U_z_values[rec_v2d] = (
        U_at_current_vertices[:, 1]
    )

    U_s_CG1.vector().set_local(
        U_s_values
    )
    U_s_CG1.vector().apply("insert")

    U_z_CG1.vector().set_local(
        U_z_values
    )
    U_z_CG1.vector().apply("insert")

    U_CG1 = Function(
        V1_recovery
    )

    component_assigner = FunctionAssigner(
        V1_recovery,
        [
            P1_recovery,
            P1_recovery,
    ...
    # At 650 s the ramp is fully on.  Re-impose the exact current
    # CG2 Dirichlet values, including mid-edge boundary DOFs.
    U_in.ramp = 1.0
    U_ret.ramp = 1.0
    ...
    # Initialise companion state Functions consistently.
    U_new.assign(U_n)
    U_star.assign(U_n)
    ...
    # Rebuild cumulative scalar accounting.
    
    # Inflow is reconstructed from the exact discrete source ramp.
    # The final conservative scalar scheme obeyed
    
    #     stored = cumulative_in - cumulative_out
    
    # to numerical precision, so use the exact saved stored mass
    # to infer the cumulative outflow at the recovery point.
    
    stored_fresh_volume = float(
        np.dot(
            cell_volumes,
            fresh_n.vector().get_local()
        )
    )

    cumulative_in = (
        cumulative_discrete_source_in(
            LEGACY_RECOVERY_STEP
        )
    )
    ...
    if cumulative_out < -1.0e-12:
        raise RuntimeError(
            "Recovered scalar volume exceeds reconstructed source input."
        )
    ...
    if rank == 0:
        print("")
        print("==============================================")
        print("RECOVERED ORIGINAL CANCELLED RUN")
        print("==============================================")
        print(
            "legacy snapshot index =",
            LEGACY_RECOVERY_INDEX
        )
        print(
            "recovered step =",
            LEGACY_RECOVERY_STEP
        ...
        print(
            "reconstructed cumulative fresh in =",
            cumulative_in,
            "m^3"
        )
        print(
            "reconstructed cumulative fresh out =",
            cumulative_out,
            "m^3"
        )
        print(
            "NOTE: pressure and DG0 fresh are recovered directly."
        )
        print(
            "NOTE: CG2 velocity is reconstructed from its saved "
            "mesh-vertex values and may show a short restart transient."
        )

    return (
        int(LEGACY_RECOVERY_STEP),
        float(
            LEGACY_RECOVERY_STEP
            * dt_value
        ),
...
if latest_checkpoint is not None:
    ...
    if rank == 0:
        print("")
        print("==============================================")
        print("RESTARTING FROM PROPER CHECKPOINT")
        print("==============================================")
    ...
    (
        ...
        cumulative_fresh_in,
        cumulative_fresh_out,
    ) = recover_legacy_visualisation_state()

    recovered_from_legacy = True

    # Immediately turn the recovered 650 s state into a proper,
    # full finite-element checkpoint.  If this job is later killed,
    # subsequent submissions will restart from this or a newer one.
    save_checkpoint(
        restart_step,
        simulation_time,
        cumulative_fresh_in,
...
if rank == 0:
    print("")
    print("==============================================")
    print("RECOVERED AXISYMMETRIC IPCS CONTINUATION FINISHED")
