# -*- coding: utf-8 -*-
# Diagnostic copy of IPSC_0.001.py; equations, boundary conditions, clipping
# and time step are unchanged. Only run length/output and diagnostics differ.
# This script has been checked statically; no diagnostic solve is claimed.
from fenics import *
import math
import csv
import json
...
# Diagnostic copy: analyse the first 60 s without changing the numerical scheme.
num_steps = 60000
write_every = 1000
...
# Set CARTESIAN_BUDGET_OUTPUT to choose another NEW/EMPTY diagnostic directory.
# The original run directory is explicitly protected.
original_folder = "/path/to/work/output_physical_plume_ipcs_1"
folder = os.environ.get(
    "CARTESIAN_BUDGET_OUTPUT", original_folder + "_budget_diagnostic_60s"
)
folder_error = None
if rank == 0:
    try:
...
# Budget diagnostics use the unmodified scalar variational forms BEFORE clipping.
...
if rank == 0:
    ...
    configuration = {
        ...
        "method": "Unchanged Oseen IPCS, implicit CG1 SUPG scalar, nodal clipping",
...
for n in range(num_steps):
    ...
    # All scalar/flux quantities below use the solved field before clipping.
    ...
    vf_postclip = float(assemble((1.0 - c_new) * dx))
    clipping_change = vf_postclip - vf_raw
...
if rank == 0:
    budget_file.flush()
    budget_file.close()
    print("Run finished. Budget CSV and configuration written to " + folder, flush=True)