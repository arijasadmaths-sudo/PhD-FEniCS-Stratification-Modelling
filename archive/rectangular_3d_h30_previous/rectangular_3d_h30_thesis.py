...
# RECTANGULAR 3D PLUME -- 30 cm HEIGHT, LONG THESIS RUN
# Legacy DOLFIN 2019.1 / PETSc / MPI

# Domain: 0.600 x 0.300 x 0.300 m, full box (no symmetry planes).
# Q=20 cc/min, nozzle diameter=2 mm, target=120 s, dt=0.004 s.
# The larger dt is a deliberate cost/temporal-accuracy compromise.
# This is a qualitative simulation; mesh/time convergence is not established.

# Graded vertical mesh: upper 63 mm has <=3.9375 mm base-layer spacing.
# Horizontal bulk spacing remains 15 mm; the central plume/nozzle are refined.
# The graded mesh avoids isotropic refinement of the entire upper volume.
# Inherited DG0 jump diffusion D/CellDiameter is approximate on this mesh;
# D=1e-7 m^2/s is a modelling value, not a verified molecular salt diffusivity.
# The nozzle is flush, and the distributed floor return approximates outflow.

# Main outputs: concentration every 1 s, velocity/pressure every 5 s,
# diagnostics every 0.5 s, checkpoints every 10 s or 1800 wall seconds.
# All accepted frames before a stop are retained in per-job segment folders.
# Re-submit the launcher, or use its bounded --chain 3 option, to resume.
# No old 0.20 m / 0.45 m domain checkpoint is imported.
...
dt_default = min(4.0e-3, 4.0e-3 * 20.0 / Q_cc_per_min)
...
# At dt=0.004 all of these intervals are represented exactly.
...
budget_relative_tolerance = 0.01
...
default_output_root = (
    "results_rectangular_3d_h30_thesis_Q{}ccmin_eps{}_dt{}".format(
        flow_tag,
        perturbation_tag,
        dt_tag,
    )
...
solver_schema = "rectangular_3d_h30_thesis_20260907a"
mesh_recipe = (
    "box_40x20x33;graded_z_14x015_012_009_006_16x0039375;"
    "plume_0p020_0p010;nozzle_0p006_0p0035_0p002_0p002"
)
...
def validate_checkpoint_metadata(meta, check_mesh=True):
    ...
    if not math.isfinite(float(meta["expected_fresh_physical"])):
...
# 15 mm bulk spacing; graded vertical layers across the full ceiling.
nx_base = 40
ny_base = 20
z_layers = np.concatenate((
    np.linspace(0.0, 0.210, 15),
    np.array([0.222, 0.231, 0.237]),
    np.linspace(0.237, H_tank, 17)[1:],
))
nz_base = len(z_layers) - 1
if (
    nz_base != 33 or abs(z_layers[0]) > 1e-15
    or abs(z_layers[-1] - H_tank) > 1e-15
    or np.any(np.diff(z_layers) <= 0)
):
    raise RuntimeError("Invalid graded vertical mesh.")
if not np.allclose(np.diff(z_layers)[-16:], 0.0039375, atol=1e-14, rtol=0):
...
root_print("Ceiling refinement begins at z =", z_layers[-17], "m")
...
def write_configuration_and_diagnostics_header():
    ...
    if rank == 0:
        try:
            configuration = {
                "model": "30 cm rectangular thesis run with distributed floor return",
...
for step in range(start_step + 1, end_step + 1):
    ...
    if (not math.isfinite(budget_relative_error)
            or (t_now >= 1.0 and budget_relative_error > budget_relative_tolerance)):
        raise RuntimeError(
            "Scalar budget error exceeds 1% at t={}: measured={}, expected={}.".format(
                t_now, fresh_volume, expected_fresh_physical
            )
        )
