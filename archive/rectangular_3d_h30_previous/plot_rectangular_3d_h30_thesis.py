...
Example (after the run, using one ordinary Python process, not srun):
  python3 plot_rectangular_3d_h30_thesis.py --results RESULTS_DIRECTORY
...
def plot_panels(chosen, prepared, destination, vmin, name):
    ...
    if artist is not None:
        ...
        bar.set_label("Concentration $c$ (0: fresh, 1: initial salt water)")
...
def main():
    ...
    parser.add_argument("--pattern", default="segments/*/c_dg0.xdmf",
                        help="Glob relative to --results; default: segments/*/c_dg0.xdmf")
    ...
    plot_panels(chosen, prepared, destination, 0, "rectangular_3d_concentration_full")
    plot_panels(chosen, prepared, destination, 0.97, "rectangular_3d_concentration_contrast_097")
    ...
    ax.set_xlabel("Horizontally averaged freshwater fraction, $100(1-c)$ (%)")
    ax.set_ylabel("$z$ (mm)")
    ...
    with (destination / "rectangular_3d_horizontal_profiles.csv").open("w", newline="") as stream:
        ...
        writer.writerow(["time_s", "z_m", "depth_below_ceiling_m", "area_mean_freshwater_fraction"])
    ...
    for choice in chosen:
        ...
        if frame is not None:
            ...
            entry.update(actual_time_s=frame["time"], source_xdmf=frame["xdmf"],
                         ...
                         freshwater_mL=data["freshwater_mL"], c_min=data["c_min"], c_max=data["c_max"])
        selected.append(entry)
    summary = dict(available_time_range_s=[frames[0]["time"], frames[-1]["time"]],
                   ...
                   notes=["Exact tetrahedron section polygons; no nodal smoothing.",
                          ...
                          "Profiles and freshwater integrals use raw values without clipping.",
